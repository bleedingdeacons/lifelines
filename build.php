...
/**
 * Build Script for LifeLines WordPress Plugin
 *
...
class PluginBuilder
{
    ...
    private string $pluginName = 'lifelines';
    ...
    {
        $mainFile = $this->pluginDir . DIRECTORY_SEPARATOR . 'lifelines.php';
        if (file_exists($mainFile)) {
            $content = file_get_contents($mainFile);
            if (preg_match('/Version:\s*([0-9.]+\s*\w*)/', $content, $matches)) {
                return trim($matches[1]);
            }
            if (preg_match("/define\s*\(\s*'LIFELINES_VERSION'\s*,\s*'([^']+)'/", $content, $matches)) {
                return $matches[1];
            }
        }
        return '0.0.1';
     ...
     * the standalone guide (assets/docs/lifelines.html) and the in-admin help page
     * (templates/help-page.php).
     */
    private function syncDocsVersion(): void
    {
        $docs = [
            'assets' . DIRECTORY_SEPARATOR . 'docs' . DIRECTORY_SEPARATOR . 'lifelines.html',
...
LifeLines WordPress Plugin Build Script
====================================
Platform: {$platform}
